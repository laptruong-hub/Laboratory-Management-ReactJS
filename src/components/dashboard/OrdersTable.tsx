...
export const OrdersTable: React.FC<OrdersTableProps> = ({ rows }) => {
  ...
  const filtered = useMemo(() => {
    return rows.filter(r => {
      ...
      const matchesSearch = !q || r.id.toLowerCase().includes(q) || r.patientName.toLowerCase().includes(q);
      ...
      <Header>
        ...
        <FiltersRow>
          {[
            ...
            <TabButton key={item.key} $active={filter === (item.key as any)} onClick={() => setFilter(item.key as any)}>
              {item.label}
            </TabButton>
          ))}
          ...
          <tbody>
            {filtered.map(r => (
              <Tr key={r.id}>
                ...
                <Td style={{ color: r.overdue ? '#B91C1C' : '#6B7280', fontWeight: r.overdue ? 600 : 400 }}>
                  {r.overdue && <AlertTriangle size={14} style={{ marginRight: 6, verticalAlign: 'text-bottom' }} />}
                  {r.dueAt}
                </Td>
                <Td>
                  <Actions>
